...
from services.elasticsearch import get_vessel_identity, get_vessel_pattern, get_vessel_track
...
async def vessel_pattern(request: Request, mmsi: str, days: int = 365):
    ...
    result = await get_vessel_pattern(request.app.state.es, mmsi, days)
    if result["x_labels"]:
